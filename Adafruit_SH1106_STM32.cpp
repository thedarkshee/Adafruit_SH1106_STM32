...
void Adafruit_SH1106_STM32::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
// initializer for I2C
Adafruit_SH1106_STM32::Adafruit_SH1106_STM32() :
...
void Adafruit_SH1106_STM32::begin() {
	
	int8_t vccstate = SH1106_EXTERNALVCC;
	
/*
	HWire.begin();

	HWire.beginTransmission(SH1106_I2C_ADDRESS);

	HWire.write(OLED_CONTROL_BYTE_CMD_STREAM);

	HWire.write(OLED_CMD_DISPLAY_OFF);
	HWire.write(OLED_CMD_SET_MUX_RATIO);
	HWire.write(0x3F);
	HWire.write(OLED_CMD_SET_DISPLAY_OFFSET);
	HWire.write(0x00);
	HWire.write(OLED_CMD_SET_DISPLAY_START_LINE);
	HWire.write(OLED_CMD_SET_SEGMENT_REMAP);
	HWire.write(OLED_CMD_SET_COM_SCAN_MODE);
	HWire.write(OLED_CMD_SET_COM_PIN_MAP);
	HWire.write(0x12);
	HWire.write(OLED_CMD_SET_CONTRAST);
	HWire.write(0x7F);
	HWire.write(OLED_CMD_DISPLAY_RAM);
	HWire.write(OLED_CMD_DISPLAY_NORMAL);
	HWire.write(OLED_CMD_SET_DISPLAY_CLK_DIV);
	HWire.write(0x80);
	HWire.write(OLED_CMD_SET_CHARGE_PUMP);
	HWire.write(0x14);
	HWire.write(OLED_CMD_SET_PRECHARGE);
	HWire.write(0x22);
	HWire.write(OLED_CMD_SET_VCOMH_DESELCT);
	HWire.write(0x30);
	HWire.write(OLED_CMD_DISPLAY_ON);
...
void Adafruit_SH1106_STM32::invertDisplay(uint8_t i) {
  if (i) {
...
void Adafruit_SH1106_STM32::SH1106_command(uint8_t c) { 
    // I2C
...
void Adafruit_SH1106_STM32::SH1106_data(uint8_t c) {
...
void Adafruit_SH1106_STM32::display(void) {
	...
	for ( i = 0; i < height; i++) {  //height = 8, so 8 pages
        ...
        for( j = 0; j < 8; j++){        //send in 8 chunks of 16 bytes (2 squares) each
			HWire.beginTransmission(SH1106_I2C_ADDRESS);  //Interestingly you can only send 16 bytes per 0x40 call
            HWire.write(0x40);
            for ( k = 0; k < width; k++, p++) {   //width = 16, so buffer is in blocks of 16 bytes
...
// clear everything
void Adafruit_SH1106_STM32::clearDisplay(void) {
  memset(buffer, 0, (SH1106_LCDWIDTH*SH1106_LCDHEIGHT/8));
}

void Adafruit_SH1106_STM32::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
  boolean bSwap = false;
  switch(rotation) { 
    case 0:
...
void Adafruit_SH1106_STM32::drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
void Adafruit_SH1106_STM32::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
  bool bSwap = false;
  switch(rotation) { 
    case 0:
      break;
...
void Adafruit_SH1106_STM32::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
